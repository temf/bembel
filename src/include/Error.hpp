...
// information.
#ifndef __BEMBEL_Error_
#define __BEMBEL_Error_
...
inline Eigen::Matrix<double, Eigen::Dynamic, 1> errors(
    const Eigen::MatrixXd &pot, const Eigen::MatrixXd &grid,
...
inline Eigen::Matrix<double, Eigen::Dynamic, 1> errors(
  ...
  for (int i = 0; i < gridsz; i++) {
    errors(i) = std::abs(pot(i) - fun((grid.row(i).transpose()).eval(), kappa));
  }
  return errors;
}
...
inline double maxPointwiseError(
  ...
  for (int i = 0; i < gridsz; i++) {
    double tmp =
        std::abs(pot(i) - fun((grid.row(i).transpose()).eval(), kappa));
    error = tmp > error ? tmp : error;
  }
  return error;
}
...
inline double maxPointwiseError(
  ...
  for (int i = 0; i < gridsz; i++) {
    double tmp = (pot.row(i) - fun(grid.row(i), kappa).transpose()).norm();
    error = tmp > error ? tmp : error;
  }
  return error;
}
