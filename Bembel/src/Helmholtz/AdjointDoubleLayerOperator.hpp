...
// define traits
class HelmholtzAdjointDoubleLayerOperator;

template <>
struct LinearOperatorTraits<HelmholtzAdjointDoubleLayerOperator> {
 ...
 */
class HelmholtzAdjointDoubleLayerOperator
    : public LinearOperatorBase<HelmholtzAdjointDoubleLayerOperator> {
 ...
 public:
  ...
  void evaluateIntegrand_impl(
      ...
      Eigen::Matrix<typename LinearOperatorTraits<
                        HelmholtzAdjointDoubleLayerOperator>::Scalar,
  ...
  std::complex<double> evaluateKernelGrad(const Eigen::Vector3d &x,
    ...
    return -c.dot(x_n) * std::exp(-i * wavenumber_ * r) *
