...
class LaplaceHypersingularOperator
 ...
 public:
  ...
  Eigen::Matrix<double, 2, 2> evaluateFMMInterpolation_impl(
    ...
    intval.setZero();
    intval(0, 0) = kernel * p1.get_f_dy().dot(p2.get_f_dy());
    intval(0, 1) = -kernel * p1.get_f_dy().dot(p2.get_f_dx());
    intval(1, 0) = -kernel * p1.get_f_dx().dot(p2.get_f_dy());
    intval(1, 1) = kernel * p1.get_f_dx().dot(p2.get_f_dx());
