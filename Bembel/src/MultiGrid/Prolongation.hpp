...
Eigen::SparseMatrix<double> prolongationMatrix(
  ...
  const Eigen::SparseMatrix<double> fine_glue_mat =
      fine_glue.get_glue_matrix();
  const Eigen::SparseMatrix<double> coarse_glue_mat =
      coarse_glue.get_glue_matrix();

  Eigen::VectorXd degree_vector =
      fine_glue_mat.transpose() *
      Eigen::VectorXd::Ones(fine_glue_mat.rows());
