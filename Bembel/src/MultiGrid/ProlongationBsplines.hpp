...
std::tuple<Eigen::MatrixXd, unsigned int> nonZerosInfoRow(
  ...
  const unsigned int knots_size = static_cast<unsigned int>(knots.size());
  while (d + 1 < knots_size &&
         uniform_refined_knots[row_id] >= knots[d]) ++d;
  if (d > 0) --d;
  ...
  for (int i = 0; i < num_fine_basis; ++i) {
    ...
    for (int j = 0; j < polynomial_degree + 1; ++j) {
      tripletList.push_back(
          Triplet(i, d - polynomial_degree + j, non_zero_alpha(0, j)));
    }
  }
