...
class MaxwellSingleLayerOperator
 ...
 public:
  ...
  void evaluateIntegrand_impl(
    ...
    auto kernel_evaluation =
        evaluateKernel(p1.get_f(), p2.get_f()) * p1.get_w() * p2.get_w();
    ...
    super_space.addScaledVectorBasisInteraction(
        intval, integrand_vector, p1.get_xi(), p2.get_xi(), p1.get_f_dx(),
        p1.get_f_dy(), p2.get_f_dx(), p2.get_f_dy());

    // divergence part: multiply shape functions with integrand and add to
    // buffer
    super_space.addScaledVectorBasisDivergenceInteraction(
        intval, integrand_divergence, p1.get_xi(), p2.get_xi());
  ...
  Eigen::Matrix<std::complex<double>, 4, 4> evaluateFMMInterpolation_impl(
    ...
    // evaluate kernel
    auto kernel = evaluateKernel(p1.get_f(), p2.get_f());

    // interpolation
    Eigen::Matrix<std::complex<double>, 4, 4> intval;
    intval.setZero();
    intval(0, 0) = kernel * p1.get_f_dx().dot(p2.get_f_dx());
    intval(0, 2) = kernel * p1.get_f_dx().dot(p2.get_f_dy());
    intval(2, 0) = kernel * p1.get_f_dy().dot(p2.get_f_dx());
    intval(2, 2) = kernel * p1.get_f_dy().dot(p2.get_f_dy());
