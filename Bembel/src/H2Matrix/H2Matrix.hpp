  ...
  static void scaleAndAddTo(Dest& dst, const H2Matrix<ScalarT>& lhs,
    ...
    for (int col_component = 0; col_component < vector_dimension;
      ...
      for (int row_component = 0; row_component < vector_dimension;
        ...
        std::vector<Matrix<ScalarT, Dynamic, Dynamic>> long_dst_backward;
        for (int i = 0; i < long_rhs_forward.size(); ++i)
          long_dst_backward.push_back(Matrix<ScalarT, Dynamic, Dynamic>::Zero(
        ...
        for (auto leaf =
                 lhs.get_block_cluster_tree()(row_component, col_component)
                     .clbegin();
             leaf !=
             lhs.get_block_cluster_tree()(row_component, col_component).clend();
             ++leaf) {
          switch ((*leaf)->get_cc()) {
            // deal with matrix blocks
            case Bembel::BlockClusterAdmissibility::Dense: {
              long_dst.segment((*leaf)->get_row_start_index(),
            ...
            case Bembel::BlockClusterAdmissibility::LowRank: {
              const Bembel::ElementTreeNode* cluster1 = (*leaf)->get_cluster1();
              const Bembel::ElementTreeNode* cluster2 = (*leaf)->get_cluster2();
              ...
              long_dst_backward[fmm_level].col(cluster1_col) +=
                  (*leaf)->get_leaf().get_F() *
                  long_rhs_forward[fmm_level].col(cluster2_col);
            } break;
            // this leaf is not a low-rank block and not a dense block, thus it
            // is not a leaf -> error
            default:
              assert(0 && "This should never happen");
              break;
        ...
        long_dst += Bembel::H2Multipole::backwardTransformation(
            moment_matrix[row_component], transfer_matrices,
            max_level - min_cluster_level - 1, long_dst_backward);
