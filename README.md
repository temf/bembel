...
design of the Eigen library. If Bembel is used, only these module files should be included. These modules are the following:
    ...
    * `H2Matrix` handles the compression of the large, densely populated matrices. The algorithm was introduced in [[5]](#5) and was generalized to the Maxwell case in [[8]](#8).
